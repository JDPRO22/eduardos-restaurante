...
const catNomes = {
  food: "Comida",
  drinks: "Bebida",
  savory: "Porção Salgada",
  sweet: "Porção Doce",
  snacks: "Lanches",
};
const getSenha = () => localStorage.getItem(SENHA_KEY) || "1234";
...
window.acessoAdmin = () => {
  ...
  setTimeout(() => document.getElementById("input-senha-acesso").focus(), 100);
};
window.confirmarSenhaAdmin = () => {
  if (document.getElementById("input-senha-acesso").value === getSenha()) {
    fecharModal("modal-senha");
    irPara("tela-admin");
  } else {
    document.getElementById("input-senha-acesso").style.border =
      "2px solid red";
    setTimeout(
      () => (document.getElementById("input-senha-acesso").style.border = ""),
      1500,
...
document
  .getElementById("input-senha-acesso")
  .addEventListener("keydown", (e) => {
    if (e.key === "Enter") window.confirmarSenhaAdmin();
  });
...
window.trocarSenha = () => {
  ...
  const senhaAtual =
    tipo === "cozinha"
      ? localStorage.getItem("df_senha_cozinha") || "cozinha"
      : getSenha();
  const atual = document.getElementById("senha-atual").value,
    nova = document.getElementById("senha-nova").value,
    conf = document.getElementById("senha-confirma").value;
  ...
  mostrarToast(
    `✅ Senha ${tipo === "cozinha" ? "da cozinha" : "do admin"} alterada!`,
  );
  ["senha-atual", "senha-nova", "senha-confirma"].forEach(
    (id) => (document.getElementById(id).value = ""),
  );
};
window.abrirModalTrocarSenha = (tipo) => {
  document.getElementById("trocar-senha-tipo").value = tipo;
  document.getElementById("trocar-senha-titulo").innerText =
    tipo === "cozinha"
      ? "🍳 Alterar Senha da Cozinha"
...
window.acessoCozinha = () => {
  document.getElementById("input-senha-cozinha").value = "";
  abrirModal("modal-senha-cozinha");
  setTimeout(() => document.getElementById("input-senha-cozinha").focus(), 100);
};
window.confirmarSenhaCozinha = () => {
  const senhaCozinha = localStorage.getItem("df_senha_cozinha") || "cozinha";
  if (document.getElementById("input-senha-cozinha").value === senhaCozinha) {
    fecharModal("modal-senha-cozinha");
    irPara("tela-cozinha");
  } else {
    document.getElementById("input-senha-cozinha").style.border =
      "2px solid red";
    setTimeout(
      () => (document.getElementById("input-senha-cozinha").style.border = ""),
      1500,
...
document
  .getElementById("input-senha-cozinha")
  .addEventListener("keydown", (e) => {
    if (e.key === "Enter") window.confirmarSenhaCozinha();
  });

let consumoSelecionado = "aqui";
window.selecionarConsumo = (tipo) => {
  consumoSelecionado = tipo;
  document
    .getElementById("opcao-aqui")
    .classList.toggle("selecionado", tipo === "aqui");
  document
    .getElementById("opcao-levar")
    .classList.toggle("selecionado", tipo === "levar");
};

window.addCarrinho = (fid) => {
  ...
  document.getElementById("modal-add-preco").innerText =
    `R$ ${p.preco.toFixed(2)}`;
...
function redimensionarImagem(file, maxW = 800, maxH = 800, quality = 0.75) {
...
async function uploadImagem(file, pasta) {
  ...
  try {
    ...
    throw new Error("Erro ao processar imagem: " + e.message);
  }
  const nome =
    pasta +
    "/" +
  ...
  try {
    ...
    console.warn("Firebase Storage falhou, usando base64 reduzida:", e.message);
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = (ev) => resolve(ev.target.result);
      reader.onerror = () => reject(new Error("Falha no fallback base64"));
...
window.adicionarProduto = async () => {
  ...
  mostrarToast("⏳ Salvando produto...");
  try {
    let foto = "logo.jpeg";
    if (fotoFile) {
      foto = await uploadImagem(fotoFile, "produtos");
    }
...
window.excluirProduto = async (fid) => {
  if (confirm("Excluir este produto?"))
    await deleteDoc(doc(db, "produtos", fid));
};
...
window.salvarEdicaoProduto = async () => {
  ...
  const promo = document.getElementById("edit-promo").value,
    fotoFile = document.getElementById("edit-foto").files[0];
  if (!nome || isNaN(preco) || preco <= 0)
    return alert("Preencha nome e preço!");
  const p = produtos.find((x) => x.firestoreId === fid);
  if (!p) return;
  ...
  try {
    const atualizado = { ...p, nome, preco, cat, promo };
    if (fotoFile) {
      atualizado.foto = await uploadImagem(fotoFile, "produtos");
    }
...
function renderCardapio() {
  ["food", "drinks", "savory", "sweet", "snacks"].forEach((cat) => {
    const div = document.getElementById(`lista-${cat}`);
    div.innerHTML = "";
    const filtrados = produtos.filter((p) => p.cat === cat);
    if (!filtrados.length) {
      div.innerHTML =
        '<p style="color:var(--muted);padding:10px 0;font-size:0.9rem;">Nenhum item nesta categoria.</p>';
      return;
    }
    filtrados.forEach((p) => {
      div.innerHTML += `<div class="card">${p.promo === "sim" ? '<span class="badge-promo">🔥 Promoção</span>' : ""}
            <img class="produto-img" src="${p.foto || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&h=300&fit=crop"}" alt="${p.nome}" loading="lazy">
...
function renderCarrinho() {
  ...
  if (!carrinho.length) {
    div.innerHTML =
      '<p style="color:var(--muted);text-align:center;padding:20px;">Carrinho vazio!</p>';
  ...
  carrinho.forEach((item, i) => {
    total += item.preco;
    const consumoLabel =
      item.consumo === "levar" ? "🥡 Para levar" : "🍽️ Consumir aqui";
    const obsHtml = item.obs
      ? `<div class="cart-item-detalhe">📝 ${item.obs}</div>`
      : "";
...
window.finalizarPedido = async () => {
  ...
  pedidoAtualFid = docRef.id;
  pedidoAtualStatus = "pendente";

  let msg = `🍽️ *RESTAURANTE DEUS É FIEL*\n━━━━━━━━━━━━━━━━━\n📌 *Pedido Nº ${num}*\n👤 *Cliente:* ${nome}\n💳 *Pagamento:* ${pag}\n🕐 *Hora:* ${pedido.hora}\n━━━━━━━━━━━━━━━━━\n`;
  carrinho.forEach((i) => {
    const consumoLabel =
      i.consumo === "levar" ? "🥡 Para levar" : "🍽️ Consumir aqui";
  ...
  const comproItens = pedido.itens
    .map((i) => {
...
function renderCozinha() {
  const grid = document.getElementById("kitchen-pedidos");
  grid.innerHTML = "";
  const pendentes = pedidos
    .filter((p) => p.status !== "entregue")
    .sort((a, b) => a.timestamp - b.timestamp);
  if (!pendentes.length) {
    grid.innerHTML = `<div class="kitchen-empty"><span>🍽️</span><p style="color:#555;">Nenhum pedido pendente.<br>Os pedidos aparecem aqui em tempo real.</p></div>`;
    return;
  }
  pendentes.forEach((p) => {
    const isNovo = Date.now() - p.timestamp < 120000,
      isPronto = p.status === "pronto";
    ...
    const itensHtml = p.itens
      .map((i) => {
        ...
        const obsHtml = i.obs
          ? `<div class="kitchen-item-obs">📝 ${i.obs}</div>`
          : "";
        return `<div class="kitchen-item"><div class="kitchen-item-nome">${i.nome}${consumoBadge}</div>${obsHtml}</div>`;
      })
      .join("");
    const obsGeralHtml = p.obsGeral
      ? `<div class="kitchen-obs-geral"><strong>📋 Obs. Geral</strong>${p.obsGeral}</div>`
      : "";
...
window.marcarPronto = async (fid) => {
  await updateDoc(doc(db, "pedidos", fid), { status: "pronto" });
  tocarSom();
};
window.marcarEntregue = async (fid) => {
  await updateDoc(doc(db, "pedidos", fid), { status: "entregue" });
};
window.excluirPedidoCozinha = async (fid) => {
  if (confirm("Remover da fila?")) await deleteDoc(doc(db, "pedidos", fid));
};
...
function renderAdmin() {
  ...
  document.getElementById("adm-vendas-total").innerText =
    `R$ ${total.toFixed(2)}`;
  ...
  document.getElementById("adm-wa-num").value =
    localStorage.getItem("df_wa") || "";
  const lp = document.getElementById("adm-lista-pedidos");
  lp.innerHTML = "";
  [...pedidos]
  ...
  if (!pedidos.length)
    lp.innerHTML =
      '<tr><td colspan="6" style="color:var(--muted);text-align:center;">Nenhum pedido ainda.</td></tr>';
  const lprod = document.getElementById("adm-lista-produtos");
  lprod.innerHTML = "";
  produtos.forEach((p) => {
...
window.zerarEtapa1 = () => {
  if (document.getElementById("zerar-senha").value !== getSenha()) {
    ...
    setTimeout(
      () => (document.getElementById("zerar-senha").style.border = ""),
      1500,
    );
...
window.confirmarZerarSistema = async () => {
  try {
    ...
    (await getDocs(collection(db, "pedidos"))).forEach((d) =>
      batch.delete(d.ref),
    );
    (await getDocs(collection(db, "produtos"))).forEach((d) =>
      batch.delete(d.ref),
    );
    await batch.commit();
    fecharModal("modal-zerar");
    mostrarToast("🗑️ Sistema zerado!");
  } catch (e) {
...
window.solicitarExclusaoPedido = (fid, id, cliente, total, hora) => {
  ...
  document.getElementById("excluir-pedido-info").innerText =
    `Pedido #${id} — ${cliente} — R$ ${parseFloat(total).toFixed(2)} — ${hora}`;
  ...
  setTimeout(
    () => document.getElementById("excluir-pedido-senha").focus(),
    120,
  );
};
...
window.confirmarExclusaoPedido = async () => {
  await deleteDoc(
    doc(db, "pedidos", document.getElementById("excluir-pedido-id").value),
  );
...
function startRealtimeListeners() {
  ...
  onSnapshot(
    query(collection(db, "pedidos"), orderBy("timestamp", "asc")),
    (snap) => {
      const novosIds = new Set(snap.docs.map((d) => d.id));
      const temNovo = snap.docs.some(
        (d) => !idsAnteriores.has(d.id) && idsAnteriores.size > 0,
      );
      ...
      if (pedidoBalcaoFid) {
        const pedBalcao = pedidos.find(
          (p) => p.firestoreId === pedidoBalcaoFid,
        );
        if (
          pedBalcao &&
          pedBalcao.status === "pronto" &&
          pedidoBalcaoStatus !== "pronto"
        ) {
          pedidoBalcaoStatus = "pronto";
          tocarSom();
          document.getElementById("balcao-notif-num").innerText =
            `#${pedBalcao.id}`;
          document.getElementById("balcao-notif-cliente").innerText =
            pedBalcao.cliente
              ? `${pedBalcao.cliente} — Pronto para retirada`
              : "Pronto para retirada";
          document.getElementById("balcao-notif-pronto").classList.add("ativa");
          document
            .getElementById("balcao-notif-pronto")
...
async function initPrimeiraCarga() {
  try {
    const [ps, pds] = await Promise.all([
      getDocs(collection(db, "produtos")),
      getDocs(query(collection(db, "pedidos"), orderBy("timestamp", "asc"))),
    ]);
    ...
    if (el) el.innerText = "Erro de conexão com Firebase";
    console.error(e);
  }
}
...
function renderRelatorio() {
  ...
  if (!Object.keys(pagMap).length) {
    tbPag.innerHTML =
      '<tr><td colspan="3" style="color:var(--muted);text-align:center;">Nenhum dado.</td></tr>';
  } else {
    Object.entries(pagMap)
  ...
  if (!lista.length) {
    tbPed.innerHTML =
      '<tr><td colspan="6" style="color:var(--muted);text-align:center;">Nenhum pedido neste dia.</td></tr>';
  } else {
    [...lista]
      .sort((a, b) => b.timestamp - a.timestamp)
      .forEach((p) => {
...
window.gerarPdfRelatorio = () => {
  ...
  const pagRows =
    Object.entries(pagMap)
      ...
      .map(
        ([pag, v]) =>
          `<tr><td>${pag}</td><td style="text-align:center;">${v.qtd}</td><td style="text-align:right;">R$ ${v.total.toFixed(2)}</td></tr>`,
      )
      .join("") ||
  ...
  const prodRows =
    Object.entries(prodMap)
      ...
      .map(
        ([nome, v]) =>
          `<tr><td>${nome}</td><td style="text-align:center;">${v.qtd}</td><td style="text-align:right;">R$ ${v.total.toFixed(2)}</td></tr>`,
      )
      .join("") ||
    '<tr><td colspan="3" style="text-align:center;color:#999;">Nenhum dado.</td></tr>';
  ...
  const pedRows =
    [...lista]
      ...
      .join("") ||
    '<tr><td colspan="6" style="text-align:center;color:#999;">Nenhum pedido.</td></tr>';
...
<div class="footer">Relatório gerado automaticamente pelo sistema · Eduardo's Restaurante</div>
...
document
  .getElementById("input-senha-balcao")
  .addEventListener("keydown", (e) => {
    if (e.key === "Enter") window.confirmarSenhaBalcao();
  });
...
window.confirmarSenhaBalcao = () => {
  if (document.getElementById("input-senha-balcao").value === getSenha()) {
    ...
    document.getElementById("input-senha-balcao").style.border =
      "2px solid red";
    setTimeout(
      () => (document.getElementById("input-senha-balcao").style.border = ""),
      1500,
...
window.selecionarConsumoBalcao = (tipo) => {
  ...
  document
    .getElementById("bp-opcao-aqui")
    .classList.toggle("selecionado", tipo === "aqui");
  document
    .getElementById("bp-opcao-levar")
    .classList.toggle("selecionado", tipo === "levar");
};

window.renderBalcaoCardapio = () => {
  const busca = (document.getElementById("balcao-busca")?.value || "")
    .toLowerCase()
    .trim();
  ...
  cats.forEach((cat) => {
    const filtrados = produtos.filter(
      (p) =>
        p.cat === cat.id && (!busca || p.nome.toLowerCase().includes(busca)),
    );
  ...
  if (!div.innerHTML)
    div.innerHTML =
      '<p style="color:var(--muted);padding:20px;text-align:center;">Nenhum produto encontrado.</p>';
};
...
window.addBalcao = (fid) => {
  ...
  document.getElementById("modal-bp-preco").innerText =
    `R$ ${p.preco.toFixed(2)}`;
...
window.confirmarAddBalcao = () => {
  ...
  carrinhoBalcao.push({
    ...p,
    _uid: Date.now() + Math.random(),
    consumo: consumoBalcaoSelecionado,
    obs,
  });
...
function renderCarrinhoBalcao() {
  ...
  if (!carrinhoBalcao.length) {
    div.innerHTML =
      '<p style="color:var(--muted);text-align:center;padding:16px 0;font-size:0.9rem;">Nenhum item adicionado.</p>';
...
window.finalizarPedidoBalcao = async () => {
  ...
  const nome =
    document.getElementById("balcao-cliente").value.trim() || "Balcão";
  ...
  document.getElementById("comanda-conteudo").innerHTML = `
        ...
        <hr class="com-divider">
        <div style="text-align:center;font-size:0.8rem;margin-top:6px;">✨ Eduardo's! ✨</div>
    `;
...
async function main() {
  ...
  if (!cfg) {
    setLoadingSub("Configuração do Firebase ausente");
    return;
  }

  ({ db, storage } = createClients(cfg.firebase));

  startRealtimeListeners();

  getDoc(doc(db, "config", "restaurante"))
    .then((d) => {
      if (d.exists() && d.data().whatsapp)
        localStorage.setItem("df_wa", d.data().whatsapp);
    })
    .catch(() => {});

  await initPrimeiraCarga();
}
